...
class Seq2SeqDataset(chainer.dataset.DatasetMixin):
    def __init__(
            self,
        ...
        if Path(source_sentence_path).exists() and Path(target_sentence_path).exists():
        ...
        self.pairs = [
            (np.array(s['encoded_tokens'], np.int32), np.array(t['encoded_tokens'], np.int32)) 
